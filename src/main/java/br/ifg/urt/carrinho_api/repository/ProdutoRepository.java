...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
    ...
    * @param id Identificador do estudante.
    ...
    // Busca produtos com preço menor que um valor
    // SELECT * FROM produtos WHERE preco < ?
    List<Produto> findByPrecoLessThan(Double preco);

    // Busca produtos em estoque e com preço entre dois valores
    ...
    // Consulta JPQL (usa o nome da classe e atributos, não a tabela)
    @Query("SELECT p FROM Produto p WHERE p.preco < :preco")
    List<Produto> buscarPorPreceMenorQue(@Param("preco") Double preco);

    // Consulta SQL nativo (usa o nome real da tabela)
